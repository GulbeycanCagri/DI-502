...
def get_performance_metrics(
    tag: str, start_time: float, response: str, prefix: str = "", **extra
):
    end_time = time.time()
    ...
    for enum, golden_file in enumerate(os.listdir(evaluation_json_path)):
        ...
        for item in tqdm(golden_dataset):
            ...
            for qa in qa_pairs:
                ...
                rag_answer, perf_metrics = generate_llama_answer(
                    question, retrieved_contexts, sample_idx
                )
...
if __name__ == "__main__":
    ...
    evaluation_data, results_performance = evaluate_rag_pipeline(
        "data/datasets/test_SEC_10K_dataset"
    )
    if not evaluation_data["question"]:
        raise ValueError(
            "No evaluation samples were generated. Check the evaluation dataset."
        )
    ...
    # return mean scores
    final_results_evaluation["faithfulness"] = sum(
        list(results_evaluation["faithfulness"])
    ) / len(evaluation_data["question"])
    final_results_evaluation["answer_relevancy"] = sum(
        list(results_evaluation["answer_relevancy"])
    ) / len(evaluation_data["question"])
    final_results_evaluation["context_relevance"] = sum(
        list(results_evaluation["nv_context_relevance"])
    ) / len(evaluation_data["question"])

    # make nans to 0
    final_results_evaluation = {
        k: (0 if (isinstance(v, float) and (v != v)) else v)
        for k, v in final_results_evaluation.items()
    }

    # return std scores
    final_results_evaluation["faithfulness_std"] = torch.std(
        torch.tensor(results_evaluation["faithfulness"])
    ).item()
    final_results_evaluation["answer_relevancy_std"] = torch.std(
        torch.tensor(results_evaluation["answer_relevancy"])
    ).item()
    final_results_evaluation["context_relevance_std"] = torch.std(
        torch.tensor(results_evaluation["nv_context_relevance"])
    ).item()
    ...
    final_results_performance["response_time_sec"] = sum(
        list(results_performance["response_time_sec"])
    ) / len(evaluation_data["question"])
    final_results_performance["throughput_tokens_per_sec"] = sum(
        list(results_performance["throughput_tokens_per_sec"])
    ) / len(evaluation_data["question"])
    final_results_performance["gpu_memory_MB"] = sum(
        list(results_performance["gpu_memory_MB"])
    ) / len(evaluation_data["question"])

    # return std scores
    final_results_performance["response_time_sec_std"] = torch.std(
        torch.tensor(results_performance["response_time_sec"])
    ).item()
    final_results_performance["throughput_tokens_per_sec_std"] = torch.std(
        torch.tensor(results_performance["throughput_tokens_per_sec"])
    ).item()
    final_results_performance["gpu_memory_MB_std"] = torch.std(
        torch.tensor(results_performance["gpu_memory_MB"])
    ).item()
